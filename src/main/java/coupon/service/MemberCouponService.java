...
public class MemberCouponService {
    ...
    public void issue(Long memberId, Long couponId, int maxCouponCount) {
        long existCouponCount = memberCouponRepository.countByMemberIdAndCouponId(memberId, couponId);
        if (existCouponCount >= maxCouponCount) {
            throw new IllegalStateException("쿠폰 발급은 최대 " + maxCouponCount + "개까지만 가능합니다.");
        }
